  ...
  const handleTestLogin = async () => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        throw new Error(result.error?.message || 'Test login failed')
      }
      
      // Navigate to workspace on success
      console.log('Test login successful, redirecting to workspace...')
      router.push('/workspace')
    } catch (err) {
    ...
    <div className="flex min-h-screen flex-col items-center justify-center p-4">
      <div className="bg-[#1E4B8F] w-full max-w-md p-8 rounded-lg shadow-xl">
        ...
        <button
          ...
          className="mt-4 w-full bg-green-600 text-white py-2 rounded font-bold text-sm hover:bg-green-700 transition-all"
        >
          Use Test Login (test@example.com)
        </button>
        
        {/* Comment out Google Auth
        <div className="mt-4">
          <button
